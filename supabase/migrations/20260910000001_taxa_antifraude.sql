-- ============================================================================
-- Taxa de antifraude: custo fixo por PEDIDO aprovado que foi pago no
-- cartão. Pix não passa por análise antifraude, então não tem essa taxa.
--
-- Mesma mecânica da taxa fixa do Pix: é por pedido, não por item, então
-- fica rateada proporcionalmente ao faturamento entre os itens do mesmo
-- pedido — somando exatamente o valor configurado por pedido no cartão.
-- Campo editável em "Taxas de venda" (default R$ 1,00).
-- ============================================================================

alter table sale_fee_rates add column if not exists taxa_antifraude_fixo numeric(10,2) not null default 1.00;
...
select
  ...
  round(
    ...
    + case when sr.payment_method = 'pix' then fr.taxa_gateway_pix_fixo else fr.taxa_antifraude_fixo end
      * sr.gross_amount / nullif(ot.order_gross, 0)
  , 2) as sale_cost,
