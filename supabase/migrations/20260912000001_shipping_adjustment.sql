-- ============================================================================
-- Diferença de frete: reajuste de conferência da Melhor Envio — quando o peso
-- ou a dimensão reais do pacote saem diferentes do que foi declarado, a ME
-- debita a diferença dias depois da postagem. Também cobre etiqueta comprada
-- direto no site da ME (fora do fluxo do mm-etiquetas).
--
-- order_shipping.cost_adjustment = soma acumulada desses débitos por pedido.
-- Entra como custo de frete na DRE, junto com o cost, rateado por item.
-- ============================================================================
...
alter table order_shipping
  ...
  add column if not exists adjustment_synced_at timestamptz;
